...
if __name__ == "__main__":
    ...
    print(f'[{args.model_name_or_path}] Alignment:', align_loss(q1_emb, q2_emb).numpy())
    
    # Uniformity
    q_emb = get_sentence_embedding_uniform(uniform_csv, model) # train4_uniform.csv 사용
    print(f'[{args.model_name_or_path}] Uniformity:', uniform_loss(q_emb).numpy())

    q_emb = get_sentence_embedding_uniform_all(uniform_all_csv, model) # final_master_dataset.csv 사용
    print(f'[{args.model_name_or_path}] Uniformity (All):', uniform_loss(q_emb).numpy())
