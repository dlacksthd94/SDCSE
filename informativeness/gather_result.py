                ...
                for epoch in range(1, 3):
                    for max_len in [32]:
                        ...
                        for lambda_w in list_lambda_w:
                            for pt_type in ['mask_token', 'unk_token', 'pad_token', 'dropout', 'constituency_parsing', 'none']:
                                                ...
                                                for pooler in ['wp']:
                                                    ...
                                                    for metric in ['stsb']:
                                                        for margin in ['0e-0', '1e-0', '1e-1']:
                                                            for lambda_w2 in ['0e-0', '5e-3']:
                                                                for mask_ratio in ['0e-0', '3e-1']:
                                                                    try:
                                                                        # plm, bs, lr, epoch, seed, max_len, lambda_w, pt_type, pt_num, pt_step, loss, pooler, metric, margin, lambda_w2, mask_ratio = 'bert_base', 128, '7e-6', 2, 0, 32, '0e-0', 'none', 0, 0, 'mse', 'wp', 'stsb', '0e-0', '5e-3', '3e-1'
                                                                        # plm, bs, lr, epoch, seed, max_len, lambda_w, pt_type, pt_num, pt_step, loss, pooler, metric, margin, lambda_w2, mask_ratio = 'bert_base', 128, '3e-5', 1, 0, 32, '1e-0', 'dropout', 1, 2, 'margin', 'wp', 'stsb', '1e-1', '0e-0', '0e-0'
                                                                        ...
                                                                        if not os.path.exists(result_path):
                                                                            result_path = os.path.join(root_path_another, f'my-unsup-{ENCODER.lower()}-{dict_plm[plm]}_{bs}_{lr}_{epoch}_{seed}_{max_len}_{lambda_w}_{pt_type}_{pt_num}_{pt_step}_{loss}_{pooler}_{metric}_{margin}_{lambda_w2}_{mask_ratio}', 'eval_results.txt')
                                                                        # if not os.path.exists(result_path):
                                                                        ...
                                                                        result = [plm, bs, lr, epoch, max_len, lambda_w, pt_type, pt_num, pt_step, loss, pooler, metric, margin, seed, lambda_w2, mask_ratio, round(df_temp['score'][0] * 100, 2), round(df_temp['score'][1], 2)]
                                                                        list_result.append(result)
                                                                    except:
                                                                        pass
    df = pd.DataFrame(list_result, columns=['plm', 'bs', 'lr', 'epoch', 'max_len', 'lambda_w', 'pt_type', 'pt_num', 'pt_step', 'loss', 'pooler', 'metric', 'margin', 'seed', 'lambda_w2', 'mask_ratio', 'sts', 'transfer'])
...
def result_eval(*groupby):
    root_path = os.path.join(os.path.expanduser('~'), 'PAPER', ENCODER, 'informativeness/result/evaluation', ENCODER.lower(), RESULT_FOLDER)
                                                            ...
                                                            for metric in ['stsb']:
                                                                for margin in ['0e-0', '1e-0', '1e-1']:
                                                                    for lambda_w2 in ['0e-0', '5e-3']:
                                                                        for mask_ratio in ['0e-0', '3e-1']:
                                                                            try:
                                                                                # plm, mode, taskset, bs, lr, epoch, seed, max_len, lambda_w, pt_type, pt_num, pt_step, loss, pooler, metric, margin, lambda_w2, mask_ratio = 'bert_base', 'test', 'full', 128, '7e-6', 2, 0, 32, '0e-0', 'none', 0, 0, 'mse', 'wp', 'stsb', '0e-0', '5e-3', '3e-1'
                                                                                if ENCODER == 'SimCSE':
                                                                                    ...
                                                                                    result_path = os.path.join(root_path, f'{mode}_{taskset}_unsup_{ENCODER.lower()}_{plm}_{bs}_{lr}_{epoch}_{seed}_{max_len}_{lambda_w}_{pt_type}_{pt_num}_{pt_step}_{loss}_{pooler}_{metric}_{margin}_{lambda_w2}_{mask_ratio}.txt')
                                                                                assert os.path.exists(result_path)
                                                                                ...
                                                                                result = [mode, taskset, plm, bs, lr, epoch, max_len, lambda_w, pt_type, pt_num, pt_step, loss, pooler, metric, margin, lambda_w2, mask_ratio, seed] + list(map(float, list_score))
                                                                                
                                                                                dict_result[result_path] = result
                                                                            except:
                                                                                pass

    df = pd.DataFrame(dict_result.values(), columns=['mode', 'taskset', 'plm', 'bs', 'lr', 'epoch', 'max_len', 'lambda_w', 'pt_type', 'pt_num', 'pt_step', 'loss', 'pooler', 'metric', 'margin', 'lambda_w2', 'mask_ratio', 'seed'] + list_sts + list_transfer + ['sts', 'transfer'])
...
result_dev('plm', 'bs', 'loss', 'lambda_w', 'pt_type', 'pooler', 'pt_num', 'pt_step', 'margin', 'lambda_w2', 'mask_ratio')[1]
result_eval('mode', 'taskset', 'plm', 'bs', 'loss', 'lambda_w', 'pt_type', 'pooler', 'pt_num', 'pt_step', 'margin', 'lambda_w2', 'mask_ratio')[1].loc[:, ['sts', 'transfer']].round(1)
