...
class RelatednessPytorch(object):
    ...
    def __init__(self, train, valid, test, devscores, config):
        ...
        if torch.cuda.is_available():
            ...
            self.loss_fn = self.loss_fn.to(f'cuda:{self.gpu_id}')
    ...
    def prepare_data(self, trainX, trainy, devX, devy, testX, testy):
        ...
        trainX = torch.from_numpy(trainX).float().to(f'cuda:{self.gpu_id}')
        trainy = torch.from_numpy(trainy).float().to(f'cuda:{self.gpu_id}')
        devX = torch.from_numpy(devX).float().to(f'cuda:{self.gpu_id}')
        devy = torch.from_numpy(devy).float().to(f'cuda:{self.gpu_id}')
        testX = torch.from_numpy(testX).float().to(f'cuda:{self.gpu_id}')
        testY = torch.from_numpy(testy).float().to(f'cuda:{self.gpu_id}')
    ...
    def trainepoch(self, X, y, nepoches=1):
        ...
        for _ in range(self.nepoch, self.nepoch + nepoches):
            ...
            for i in range(0, len(X), self.batch_size):
                # forward
                idx = torch.from_numpy(permutation[i:i + self.batch_size]).long().to(f'cuda:{self.gpu_id}')
                Xbatch = X[idx]
                ybatch = y[idx]
