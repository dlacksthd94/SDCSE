...
class PyTorchClassifier(object):
    ...
    def prepare_split(self, X, y, validation_data=None, validation_split=None):
        ...
        device = torch.device('cpu') if self.cudaEfficient else torch.device(f'cuda:{self.gpu_id}')
        ...
        for _ in range(self.nepoch, self.nepoch + epoch_size):
            ...
            for i in range(0, len(X), self.batch_size):
                ...
                if self.cudaEfficient:
                    Xbatch = Xbatch.to(f'cuda:{self.gpu_id}')
                    ybatch = ybatch.to(f'cuda:{self.gpu_id}')
    ...
    def score(self, devX, devy):
        ...
        if not isinstance(devX, torch.cuda.FloatTensor) or self.cudaEfficient:
            devX = torch.FloatTensor(devX).to(f'cuda:{self.gpu_id}')
            devy = torch.LongTensor(devy).to(f'cuda:{self.gpu_id}')
        with torch.no_grad():
...
class MLP(PyTorchClassifier):
    def __init__(self, params, inputdim, nclasses, l2reg=0., batch_size=64,
                 seed=1111, cudaEfficient=False, gpu_id=0):
        super(self.__class__, self).__init__(inputdim, nclasses, l2reg,
                                             batch_size, seed, cudaEfficient, gpu_id)
        """
        PARAMETERS:
        ...
        self.loss_fn = nn.CrossEntropyLoss().to(f'cuda:{self.gpu_id}')
